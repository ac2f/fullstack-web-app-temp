...
const { Stocks, Faqs } = require("../models");
...
router.post("/", async (req, res) => {
    console.log(chalk.magentaBright("> REQUEST    /stocks"));
    var secret = require("../index");
    var remoteAddress = req.socket.remoteAddress.substr(7)
    var reqv = config.parseHeader(req.body);
    if ((!config.config.validateSecret || (reqv[17]==secret && remoteAddress==req.hostname))) {
        console.log(chalk.redBright(`> REQUEST TYPE    ${reqv[0]}`));
        if (reqv[0]=="getStocks"){ 
            res.send(await Stocks.findAll());
        } else if (reqv[0]=="getStocksById") {
            var temp = await Stocks.findAll();
            for (var value in temp) {
                if (temp[value].itemId==reqv[10]) {
                    res.send(temp[value]);
                    return;
                };
            };
            res.send([]);
        } else if (reqv[0]=="getFaqs") {
            res.send((await Faqs.findAll({where: {itemId: reqv[10]}})));
        } else if (reqv[0]=="postStocks"){
            await Stocks.create({itemId: reqv[10], title: reqv[11], description: reqv[12], descriptionlong: reqv[13], price: reqv[14], priceDiscount: reqv[15]});
            res.send("true");
        };
    };
    res.send([]);
